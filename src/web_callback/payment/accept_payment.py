...
async def accept_new_rec_pay(message: Message,
    ...
    data_payment = await create_new_recurrent_payment(user_id=user_id,
                                                      rate_name=rate_name)

    pay_link = data_payment["payUrl"]
    reg_pay_num = data_payment["regPayNum"]
    ...
    payment_data = await db.get_payments_by_reg(reg_pay_id=reg_pay_num)
    print(payment_data)

    status_data = await get_status_recurrent_payment(reg_pay_num=payment_data["_id"])
